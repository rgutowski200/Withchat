...
def build_blueprint_insight(df=None, page="general"):
    ...
    avg_cov = average_income_coverage(df)
...
def render_confidence_meters(df):
    ...
    income_cov = min(max(average_income_coverage(df) * 100, 0), 100)
    max_wr = float(df["Withdrawal Rate"].max())
    tax_total = float(df.get("Estimated Federal Tax", pd.Series(dtype=float)).sum()) if "Estimated Federal Tax" in df.columns else 0
...
def run_projection_with_temp_retire_age(test_age):
    ...
    try:
        ...
        return {
            ...
            "Avg Income Coverage": average_income_coverage(temp_df),
...
def social_security_claim_factor(claim_age, fra=67):
    """
    Approximate SSA retirement benefit factor by claiming age.
    Assumes the amount entered in the app is the age-62 amount and FRA is 67.
    Early claiming reduction: 5/9 of 1% per month for first 36 months before FRA,
    then 5/12 of 1% per month beyond that. Delayed credits: 8% per year after FRA
    through age 70. This is an educational estimate.
    """
    try:
        claim_age = float(claim_age)
    except Exception:
        claim_age = 62.0
    claim_age = max(62.0, min(70.0, claim_age))
    months = round((claim_age - float(fra)) * 12)
    if months < 0:
        early_months = abs(months)
        first_36 = min(early_months, 36)
        extra = max(early_months - 36, 0)
        reduction = first_36 * (5 / 9 / 100) + extra * (5 / 12 / 100)
        return max(1 - reduction, 0)
    return 1 + (months * (2 / 3 / 100))


def adjusted_social_security_amount(amount_at_62, claim_age, fra=67):
    """
    Converts an age-62 Social Security estimate into an estimated benefit at
    the selected claiming age. If the user leaves the amount blank, returns 0.
    """
    amount_at_62 = max(float(amount_at_62 or 0), 0.0)
    if amount_at_62 <= 0:
        return 0.0
    factor_62 = social_security_claim_factor(62, fra=fra)
    factor_claim = social_security_claim_factor(claim_age, fra=fra)
    return amount_at_62 * (factor_claim / max(factor_62, 0.0001))


def average_income_coverage(df):
    """Average only retirement years with actual spending, so pre-retirement $0 spending does not create fake coverage percentages."""
    if df is None or df.empty or "Income Coverage Ratio" not in df.columns:
        return 0.0
    metric_df = df.copy()
    if "Total Spending" in metric_df.columns:
        metric_df = metric_df[metric_df["Total Spending"] > 0]
    if "Household Retired" in metric_df.columns:
        metric_df = metric_df[metric_df["Household Retired"].astype(bool)]
    if metric_df.empty:
        return 0.0
    return float(metric_df["Income Coverage Ratio"].replace([np.inf, -np.inf], np.nan).dropna().mean() or 0.0)


def average_withdrawal_rate(df):
    """Average withdrawal rate only over retirement years with spending."""
    if df is None or df.empty or "Withdrawal Rate" not in df.columns:
        return 0.0
    metric_df = df.copy()
    if "Total Spending" in metric_df.columns:
        metric_df = metric_df[metric_df["Total Spending"] > 0]
    ...
    if metric_df.empty:
        return 0.0
    return float(metric_df["Withdrawal Rate"].replace([np.inf, -np.inf], np.nan).dropna().mean() or 0.0)
...
def calculate_rtv_score(df):
    ...
    max_wr = float(df["Withdrawal Rate"].max())
    avg_wr = average_withdrawal_rate(df)
    income_coverage = average_income_coverage(df)
...
def run_projection():
    ...
    for age in range(int(st.session_state.current_age), int(st.session_state.end_age) + 1):
        ...
        user_ss = adjusted_social_security_amount(st.session_state.user_ss, st.session_state.user_ss_age) if age >= int(st.session_state.user_ss_age) else 0
        spouse_ss = adjusted_social_security_amount(st.session_state.spouse_ss, st.session_state.spouse_ss_age) if spouse_alive and spouse_age >= int(st.session_state.spouse_ss_age) else 0
        if st.session_state.has_spouse and not spouse_alive:
            if st.session_state.survivor_ss_strategy == "Higher benefit continues":
                ss_income = max(user_ss, adjusted_social_security_amount(st.session_state.spouse_ss, st.session_state.spouse_ss_age))
            else:
                ss_income = user_ss
        else:
        ...
        # Build/refill Bucket 1 to the selected years of full retirement spending need.
        # This now includes lifestyle spending, healthcare, and mortgage instead of lifestyle spending only.
        # If funded from Traditional assets, the refill is counted as ordinary taxable income instead of hidden as a flat 20% haircut.
        if household_retired and total_spending > 0 and float(st.session_state.bucket1_years) > 0:
            bucket1_target = total_spending * float(st.session_state.bucket1_years)
            bucket1_shortfall = max(bucket1_target - bucket1, 0)
            if bucket1_shortfall > 0 and trad > 0:
                bucket1_refill = min(trad, bucket1_shortfall)
                trad -= bucket1_refill
                bucket1 += bucket1_refill

        non_portfolio_income = ss_income + other_income

        ordinary_income_before_withdrawals = float(inc.get("taxable", 0)) + conversion + bucket1_refill
        ...
        rows.append({
            ...
            "Bucket 1 Refill": bucket1_refill,
...
def test_rtv_with_changes(**changes):
    ...
    try:
        ...
        return {
            ...
            "income_coverage": average_income_coverage(test_df),
...
def run_stress_test_scenario(
    ...
    ending = float(sim_df["End Total"].iloc[-1])
    max_wr = float(sim_df["Withdrawal Rate"].max())
    income_coverage = average_income_coverage(sim_df)
...
def build_pdf_report(df):
    ...
    starting = float(df["Start Total"].iloc[0])
    max_wr = float(df["Withdrawal Rate"].max())
    avg_income_coverage = average_income_coverage(df)
...
if active_page == PAGE_NAMES[0]:
    ...
    if safe_can_run_home:
        ...
        income_coverage_home = pct(average_income_coverage(safe_df_home))
...
if active_page == PAGE_NAMES[6]:
    ...
    if not can_run:
        ...
        c5.metric("Income Coverage", pct(average_income_coverage(df)))
        ...
        for test_age in range(max(int(st.session_state.current_age), 18), min(int(st.session_state.end_age), 75) + 1):
            ...
            if not tdf.empty:
                ...
                possible.append({
                    ...
                    "Avg Income Coverage": average_income_coverage(tdf),
...
if active_page == PAGE_NAMES[9]:
    ...
    def scenario_summary_from_data(data):
        ...
        if not projection_df.empty:
            ...
            income_coverage = average_income_coverage(projection_df)
...
if active_page == PAGE_NAMES[14]:
    ...
    if can_run and not df.empty:
        ...
        Average income coverage: {average_income_coverage(df)}
