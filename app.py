...
defaults = {
    ...
    "onboarding_step": 1,
    "blueprint_mode": "Quick Blueprint",
}
...
NAV_LABELS = {
    ...
    "Build My Blueprint": "Build My Blueprint",
}
...
def render_navigation():
    ...
    nav_page_order = ["Home", "Build My Blueprint"] + [p for p in PAGE_NAMES if p not in ["Home", "Build My Blueprint"]]
    nav_rows = [nav_page_order[i:i+5] for i in range(0, len(nav_page_order), 5)]
...
def render_onboarding_progress(step, total_steps=9):
    pct_complete = min(max(float(step) / max(float(total_steps), 1.0), 0), 1)

    st.markdown(f"""
    <div style="margin: 12px 0 22px 0;">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">
            <div style="font-weight:900;color:#0F172A;">Build My Blueprint</div>
            <div style="color:#64748B;font-size:.9rem;">Step {step} of {total_steps}</div>
        </div>
        <div style="height:12px;background:#E2E8F0;border-radius:999px;overflow:hidden;">
            <div style="height:100%;width:{pct_complete * 100}%;background:linear-gradient(90deg,#2563EB,#14B8A6);border-radius:999px;"></div>
        </div>
    </div>
    """, unsafe_allow_html=True)


def onboarding_card(title, subtitle=""):
    st.markdown(f"""
    <div style="
        border:1px solid #DBEAFE;
        border-radius:26px;
        ...
        margin-bottom:18px;
    ">
        <div style="font-size:.82rem;font-weight:950;text-transform:uppercase;letter-spacing:.08em;color:#2563EB;margin-bottom:8px;">
            Retirement Blueprint 101
        </div>
        <div style="font-size:1.75rem;font-weight:950;color:#0F172A;letter-spacing:-.035em;line-height:1.12;margin-bottom:8px;">
            {title}
        </div>
        <div style="color:#64748B;font-size:1rem;line-height:1.5;max-width:820px;">
            {subtitle}
        </div>
    </div>
    """, unsafe_allow_html=True)
...
if active_page == PAGE_NAMES[0]:
    ...
    if required_panel:
        ...
        with c_warn_btn:
            if st.button("Go to Build My Blueprint", use_container_width=True, key="home_go_guided_required"):
                go_to_page("Build My Blueprint")
    else:
...
if active_page == "Build My Blueprint":
    render_page_shell(
        "Build My Blueprint",
        "Answer one simple question at a time. The app will use your answers to create your starter retirement blueprint.",
        "✨"
    )
    ...
    render_onboarding_progress(step, total_steps)

    def onboarding_next():
        st.session_state.onboarding_step = min(total_steps, int(st.session_state.onboarding_step) + 1)
        st.rerun()

    def onboarding_back():
        st.session_state.onboarding_step = max(1, int(st.session_state.onboarding_step) - 1)
        st.rerun()
    ...
    if step == 1:
        onboarding_card(
            "Hi, let’s build your first retirement blueprint.",
        ...
        first_name = st.text_input(
            ...
            placeholder="Example: Ron",
            key="onboarding_first_name_input"
        )

        c1, c2 = st.columns([1, 1])
        with c2:
            if st.button("Next", type="primary", use_container_width=True, key="onboarding_step_1_next"):
                st.session_state.first_name = first_name.strip()
                onboarding_next()

    elif step == 2:
        name = st.session_state.get("first_name", "").strip() or "there"
        onboarding_card(
            f"Great, {name}. How old are you today?",
            "Your current age is the starting point for the retirement projection."
        )
        ...
        current_age = st.number_input(
            ...
            min_value=0,
            max_value=100,
            value=int(st.session_state.get("current_age", 0) or 0),
            step=1,
            key="onboarding_current_age_input"
        )

        c1, c2 = st.columns([1, 1])
        with c1:
            if st.button("Back", use_container_width=True, key="onboarding_step_2_back"):
                onboarding_back()
        with c2:
            if st.button("Next", type="primary", use_container_width=True, key="onboarding_step_2_next"):
                st.session_state.current_age = current_age
                onboarding_next()

    elif step == 3:
        onboarding_card(
            "Are you planning by yourself or with a spouse/partner?",
            "This helps the app know whether to include spouse age, spouse Social Security, healthcare, and survivor planning."
        )

        planning_type = st.radio(
            "Planning type",
            ["Just me", "With spouse/partner"],
            horizontal=True,
            index=1 if st.session_state.get("has_spouse", False) else 0,
            key="onboarding_planning_type"
        )

        spouse_name = st.session_state.get("spouse_name", "")
        spouse_age = int(st.session_state.get("spouse_age", 0) or 0)
        spouse_retire_age = int(st.session_state.get("spouse_retire_age", 0) or 0)
        spouse_plan_age = int(st.session_state.get("spouse_plan_age", 90) or 90)

        if planning_type == "With spouse/partner":
            spouse_name = st.text_input(
                "Spouse / partner name",
                value=spouse_name,
                placeholder="Example: Lynn",
                key="onboarding_spouse_name_input"
            )
            ...
            with s1:
                spouse_age = st.number_input(
                    ...
                    value=spouse_age,
                    step=1,
                    key="onboarding_spouse_age_input"
                )
            with s2:
                spouse_retire_age = st.number_input(
                    ...
                    min_value=0,
                    max_value=110,
                    value=spouse_retire_age if spouse_retire_age else int(st.session_state.get("retire_age", 0) or 62),
                    step=1,
                    key="onboarding_spouse_retire_age_input"
                )
            with s3:
                spouse_plan_age = st.number_input(
                    "Spouse plan-through age",
                    min_value=0,
                    max_value=120,
                    value=spouse_plan_age,
                    step=1,
                    key="onboarding_spouse_plan_age_input"
                )

        c1, c2 = st.columns([1, 1])
        with c1:
            if st.button("Back", use_container_width=True, key="onboarding_step_3_back"):
                onboarding_back()
        with c2:
            if st.button("Next", type="primary", use_container_width=True, key="onboarding_step_3_next"):
                st.session_state.has_spouse = planning_type == "With spouse/partner"
                st.session_state.spouse_name = spouse_name.strip() if planning_type == "With spouse/partner" else ""
                st.session_state.spouse_age = spouse_age if planning_type == "With spouse/partner" else 0
                st.session_state.spouse_retire_age = spouse_retire_age if planning_type == "With spouse/partner" else 0
                st.session_state.spouse_plan_age = spouse_plan_age if planning_type == "With spouse/partner" else 90
                onboarding_next()

    elif step == 4:
        onboarding_card(
            "What age do you want to retire?",
            "This is your target age. You can change it later and compare different retirement ages."
        )

        default_retire_age = int(st.session_state.get("retire_age", 0) or 0)
        ...
        retire_age = st.number_input(
            ...
            min_value=0,
            max_value=100,
            value=default_retire_age if default_retire_age else max(current_age + 1, 62),
            step=1,
            key="onboarding_retire_age_input"
        )

        c1, c2 = st.columns([1, 1])
        with c1:
            if st.button("Back", use_container_width=True, key="onboarding_step_4_back"):
                onboarding_back()
        with c2:
            if st.button("Next", type="primary", use_container_width=True, key="onboarding_step_4_next"):
                st.session_state.retire_age = retire_age
                # If spouse mode was selected and spouse retirement age was not entered, use the same retirement age as a starter estimate.
                if st.session_state.get("has_spouse", False) and int(st.session_state.get("spouse_retire_age", 0) or 0) == 0:
                    st.session_state.spouse_retire_age = retire_age
                onboarding_next()

    elif step == 5:
        onboarding_card(
            "How much do you have saved for retirement?",
            "A simple total is fine for now. The app will place 80% into traditional/pre-tax and 20% into Roth as a starter estimate."
        )
        ...
        total_savings = st.number_input(
            ...
            step=10000,
            key="onboarding_total_savings_input"
        )

        c1, c2 = st.columns([1, 1])
        with c1:
            if st.button("Back", use_container_width=True, key="onboarding_step_5_back"):
                onboarding_back()
        with c2:
            if st.button("Next", type="primary", use_container_width=True, key="onboarding_step_5_next"):
                st.session_state.traditional = int(total_savings * 0.80)
                st.session_state.roth = int(total_savings * 0.20)
                st.session_state.taxable = 0
                st.session_state.cash = 0
                onboarding_next()

    elif step == 6:
        onboarding_card(
            "How much do you expect to save each year until retirement?",
            "Include 401(k), IRA, Roth, employer contributions, or other retirement savings."
        )

        annual_contribution = st.number_input(
            "Annual retirement savings",
            min_value=0,
            value=int(st.session_state.get("annual_contribution", 0) or 0),
            step=5000,
            key="onboarding_annual_contribution_input"
        )

        c1, c2 = st.columns([1, 1])
        with c1:
            if st.button("Back", use_container_width=True, key="onboarding_step_6_back"):
                onboarding_back()
        with c2:
            if st.button("Next", type="primary", use_container_width=True, key="onboarding_step_6_next"):
                st.session_state.annual_contribution = annual_contribution
                onboarding_next()

    elif step == 7:
        onboarding_card(
            "How much do you expect to spend each month in retirement?",
            "Use your best estimate. You can build a detailed budget later."
        )

        monthly_spending = st.number_input(
            "Monthly retirement spending before healthcare",
            min_value=0,
            value=int(st.session_state.get("flat_monthly_spending", 0) or 0),
            step=500,
            key="onboarding_monthly_spending_input"
        )

        c1, c2 = st.columns([1, 1])
        with c1:
            if st.button("Back", use_container_width=True, key="onboarding_step_7_back"):
                onboarding_back()
        with c2:
            if st.button("Next", type="primary", use_container_width=True, key="onboarding_step_7_next"):
                st.session_state.budget_mode = "Flat monthly number"
                st.session_state.flat_monthly_spending = monthly_spending
                onboarding_next()

    elif step == 8:
        onboarding_card(
            "When do you plan to start Social Security?",
            "If you are not sure, use age 62 for an early estimate or age 67 for full-retirement-age planning."
        )

        c1, c2 = st.columns(2)
        with c1:
            user_ss_age = st.number_input(
                "Social Security start age",
                min_value=62,
                max_value=70,
                value=int(st.session_state.get("user_ss_age", 62) or 62),
                step=1,
                key="onboarding_user_ss_age_input"
            )
        with c2:
            user_ss = st.number_input(
                "Estimated annual Social Security",
                min_value=0,
                value=int(st.session_state.get("user_ss", 0) or 0),
                step=1000,
                key="onboarding_user_ss_input"
            )

        c1, c2 = st.columns([1, 1])
        with c1:
            if st.button("Back", use_container_width=True, key="onboarding_step_8_back"):
                onboarding_back()
        with c2:
            if st.button("Next", type="primary", use_container_width=True, key="onboarding_step_8_next"):
                st.session_state.user_ss_age = user_ss_age
                st.session_state.user_ss = user_ss
                onboarding_next()

    elif step == 9:
        onboarding_card(
            "Last question: do you want a quick or detailed blueprint?",
            "Quick Blueprint gets you to the dashboard fast. Detailed Blueprint lets you refine accounts, taxes, spouse details, healthcare, home equity, Roth conversions, and bucket strategy."
        )
        ...
        blueprint_mode = st.radio(
            ...
            key="onboarding_blueprint_mode_input"
        )

        st.markdown("""
        <div class="rb-next-box">
          <div class="rb-next-heading">You can always change this later</div>
          <div class="rb-muted">
            The wizard creates your starter blueprint. The detailed forms are still available anytime from the navigation menu.
          </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        c1, c2 = st.columns([1, 1])
        with c1:
            if st.button("Back", use_container_width=True, key="onboarding_step_9_back"):
                onboarding_back()
        with c2:
            if st.button("Create My Blueprint", type="primary", use_container_width=True, key="onboarding_finish"):
                st.session_state.blueprint_mode = blueprint_mode
                st.session_state.quick_blueprint_saved = True
                st.session_state.end_age = int(st.session_state.get("end_age", 90) or 90)
                st.session_state.growth_return = float(st.session_state.get("growth_return", 0.07) or 0.07)
                st.session_state.safe_return = float(st.session_state.get("safe_return", 0.045) or 0.045)
                st.session_state.inflation = float(st.session_state.get("inflation", 0.03) or 0.03)
                onboarding_finish()

    st.divider()
    c1, c2, c3 = st.columns(3)
    with c1:
        if st.button("Restart Setup Wizard", use_container_width=True, key="restart_onboarding"):
            st.session_state.onboarding_step = 1
            st.session_state.onboarding_complete = False
            st.rerun()
    with c2:
        if st.button("Skip to Detailed Form", use_container_width=True, key="skip_to_detailed_form"):
            go_to_page("Guided Questions")
    with c3:
        if st.button("Go to Dashboard", use_container_width=True, key="skip_to_dashboard"):
            go_to_page("Dashboard")
